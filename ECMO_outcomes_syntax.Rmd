...
```{r}
1. Check all yellow and orange fields
...
Less important:
  3.1 N-KHK diagnosis for all patients --> create a new separate "KHK" variable for chart review (do not use diag1, diag2, diag3)
  3.2 check patients with missing years
  3.3 chart review remaining providers
  
  
4. selet one major diagnosis based on all diagnoses
5. re-think ECMELLA definition: What if there is a small time overlap (e.g. 5 days ECMO, then 20mins Impella - is this ECMELLA?)
6. How to treat patients with OP?
  
7. Get ECMO and ECMELLA data for most recent patients - send request to Kalinowksy

Questions

1. By Carsten: Why are there so few cases in CCM und CVK in the final cohort? Why does CCM have more cases than CVK? Did we have access to all patients?
```
...
intervention <- ggplot(ecmo_master3, aes(x="", y= "", fill=intervention)) +
  geom_bar(stat="identity", width=1) +
  coord_polar("y", start=0) +
  theme_void()
intervention
...
reaa <- ggplot(ecmo_master3, aes(x="", y= "", fill=rea)) +
  geom_bar(stat="identity", width=1) +
  coord_polar("y", start=0) +
  theme_void()
reaa
...
table <- ecmo_master3 %>%
  ...
  summarise(
    len = mean(length_of_stay, na.rm=T)
  )
# Initialize ggplot with data
f <- ggplot(
  table, aes(x = intervention, y = len)) +
  geom_bar(stat = "identity",  fill = c("cornflowerblue", "chartreuse3", "chocolate1", "coral1", "cyan3")) +
  labs(title = "Mean Length of Stay", 
       x = "intervention",
...
table2 <- ecmo_master3 %>%
  group_by(intervention) %>%
  summarise(
    len = mean(ICU_length_of_stay, na.rm=T)
  )
# Initialize ggplot with data
g <- ggplot(
...
#create infarct (yes/no) variable manually and diag5 (chart reviewed if diag1 - diag4 were not enough to define infarct variable)
...
manual_infarct <- manual_infarct %>% select(id, infarct) #clean
...
#create parimary diagnosis variable
...
##create table
ecmo_master4_table1 <-  table1(~ campus +  shockable+ unshockable+ rosc_num +  rea +
                                 mort +  length_of_stay + ICU_length_of_stay + ecmo_time + impella_time + ef1 + ef2 +
                                 stemi+ nstemi+ mi+ cs+ khk_num+ aortdiss+ hi+ myocarditis+ lae+ pul+ sepsis+ intox | trt,
                               data= summary)

ecmo_master4_table2 <-  table1(~ unshockable+ rosc_num +  rea +
                                 mort +  length_of_stay + ICU_length_of_stay + ecmo_time + impella_time + ef1 + ef2 +
                                 stemi+ nstemi+ mi+ cs+ khk_num+ aortdiss+ hi+ myocarditis+ lae+ pul+ sepsis+ intox | trt*campus,
                               data= summary)

ecmo_master4_table3 <-  table1(~ campus + shockable+ unshockable+ rosc_num +  rea +
...
xtabs(~campus+trt,data=ecmo_master4)
...
log_mort <- glm(mort ~ trt,
          data = ecmo_master6, family = "binomial")
summary(log_mort)
