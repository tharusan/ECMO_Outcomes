...
n <- count(lab3, N2KATTEXT)

##Lactate
  lac <- lab3 %>% filter(N2KATTEXT=="Lac")
...
#Creatinine
crea <- lab3 %>% filter(N2KATTEXT %in% c("Kreatinin (Jaffé)", "Kreatinin (Jaffé) HP"))


#Troponin 
trop <- lab3 %>% filter(N2KATTEXT %in% c("Troponin-Ths", "Troponin-Ths HP", "Troponin T hs"))

#CK
ck <- lab3 %>% filter(N2KATTEXT %in% c("CK", "Creatinkinase (CK)", "Creatinkinase (CK) HP"))
...
#CK-MB
ckmb <- lab3 %>% filter(N2KATTEXT %in% c("CK-MB", "CK-MB HP"))
...
```{r}
library(gtools)
ecmo_master8$age_q4 <- quantcut(ecmo_master8$age, q = 4, na.rm = TRUE) #quartile of age
ecmo_master8$prov1_fact <- as.factor(ecmo_master8$prov1)

log_mort <- glm(mort ~ trt + age_q4 + GSCHL + unshockable + rea + ecmo_time,
                data = ecmo_master8, family = "binomial")
summary(log_mort)
exp(cbind(OR = coef(log_mort), confint(log_mort)))
...
los <- glm(length_of_stay ~ trt + age_q4 + GSCHL + unshockable + rea + ecmo_time,  family = poisson, data = ecmo_master8)
summary(los)
exp(cbind(IRR = coef(los), confint(los)))


iculos <- glm(ICU_length_of_stay ~ trt + age_q4 + GSCHL + unshockable + rea + ecmo_time,  family = poisson, data = ecmo_master8)
summary(iculos)
exp(cbind(IRR = coef(iculos), confint(iculos)))
