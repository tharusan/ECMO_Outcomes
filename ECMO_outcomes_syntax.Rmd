...
Chart review of final cohort (N = 93): ALWAYS create NEW Variable which can be merged into main dataset by id
  1. All missing values of already existing variables (see Table)
  2. Create provider variable: prov1_t1, prov2_t1, prov3_t1, prov1_t2, prov2_t2, prov3_t2
  2. Schrage paper: age, sex, lactate, SAVE score, SAPS II, previous cardiac arrest, blood pressure, heart rate, crea
  3. Additional - clarfiy with Carsten: CK, CKMB, Troponin, pH, APACHE, GCS, TISS, medication, BMI (height, weight), CCI


4. re-think ECMELLA definition: What if there is a small time overlap (e.g. 5 days ECMO, then 20mins Impella - is this ECMELLA?)
...
#data variables only contain dates of missings

#admission2 = new variable
ecmo_master7 <- ecmo_master7 %>% mutate(admission2= replace(admission2,
                                                               which((!is.na(admission) & is.na(admission2))), admission))

#transform to date
ecmo_master7$admission2 <- as.Date(ecmo_master7$admission2)
#control 
