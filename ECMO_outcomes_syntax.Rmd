...
2.2 N-KHK diagnosis for all patients --> create a separate "KHK" variable for chart review
2.3 check cases with "unclear" ECMO
2.4 check "unclear" and missing Impella
2.5 check patients with missing mortality
2.6 check patients with missing years
2.7 chart review remaining providers
```
#test test test
...
ecmo_master3_test <- ecmo_master3 %>% filter(rosc_num == 0 & (rea == "In-hospital" | rea == "Out-of-hospital")) #check all patients with unclear or missing rea

xtabs(~mort+ecmella_final,data=ecmo_master3_test)
