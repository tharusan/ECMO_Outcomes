...
lab$time<- as.POSIXlt.character(lab$N2LATIME, format="%H%M%S")
lab$time <- substr(lab$time, 11, 19)


#put date and time together
lab$datetime <- as.POSIXct(paste(lab$N2LADATUM, lab$time), format="%Y-%m-%d %H:%M:%S")

#merge with ecmo implanation date
...
lab2$time_to_lab <- difftime(lab2$datetime, lab2$`ECMO wann eingebaut?`)
...
lac <- lac[order(lac$id, lac$time_to_lab),]
lac_c<- lac %>% distinct(id, .keep_all = T)
