...
match_nomiss <- ecmo_final8 %>% select(trt, mort, age, GSCHL, bmi, rea, unshockable, lac_closest_within48h_after_ecmo, ph_closest_within48h_after_ecmo, survdays, SAVE, iabp) %>%
  na.omit()
match_nomiss$trt <- as.factor(match_nomiss$trt)

mod_match <- matchit(trt ~ age + GSCHL + bmi + rea + unshockable + lac_closest_within48h_after_ecmo + ph_closest_within48h_after_ecmo + SAVE + iabp,
