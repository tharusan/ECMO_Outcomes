...
```{r}
bb1 <- ggplot(ecmo_final10, aes(y=ef_admission_final, color=intervention)) + 
    geom_boxplot()

bb2 <- ggplot(ecmo_final10, aes(y=ef_discharge_final, color=intervention)) + 
    geom_boxplot()
...
t.test(length_of_stay2 ~ trt, data=ecmo_final10)
t.test(ICU_length_of_stay2 ~ trt, data=ecmo_final10)
t.test(ecmo_time2 ~ trt, data=ecmo_final10)
...
t.test(wscore2~ trt, data=ecmo_final10 )
...
t.test(ef_admission_final~ trt, data=ecmo_final10 )
t.test(ef_discharge_final~ trt, data=ecmo_final10)
...
mod_match <- matchit(trt ~ age + rea + unshockable  + iabp,
                     method = "nearest", data = match_nomiss)
...
table1_matched <- table1(~age  + GSCHL+ bmi + campus2 + rea+ shockable+ unshockable + length_of_stay2 + ICU_length_of_stay2 + ecmo_time2 + impella_time2 + mort + SAVE + iabp + iabp_cat + saps_chartreview +wscore2 + cad|trt, data=dta_m)
=======
...
t.test(length_of_stay2 ~ trt, data=dta_m)
t.test(ICU_length_of_stay2 ~ trt, data=dta_m)
t.test(ecmo_time2 ~ trt, data=dta_m)
