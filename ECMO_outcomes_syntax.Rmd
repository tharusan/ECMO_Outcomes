...
#merge eight from 5 &6
height <-merge(heightc5.3[,c(1,4,7,10)], heightc6.3[,c(1,4,6,9)], by="id", all=T)

#keep value that is closer to implantation time
height$height <- ifelse(is.na(height$time_to_height2.x), height$val, ifelse(is.na(height$time_to_height2.y), height$vFloat, ifelse(height$time_to_height2.x < height$time_to_height2.y, height$vFloat, height$val)))

#merge weight from 5&6
weight <- merge(weightc5.3[,c(1,4,7,10)], weightc6.3[,c(1,4,6,9)], by="id", all=T)

#keep value that is closer to implantation time
weight$weight <- ifelse(is.na(weight$time_to_weight2.x), weight$val, ifelse(is.na(weight$time_to_weight2.y), weight$vFloat, ifelse(weight$time_to_weight2.x < weight$time_to_weight2.y, weight$vFloat, weight$val)))
